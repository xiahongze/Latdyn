...
!             IF (n1 .EQ. n2) CYCLE
    ...
    SUBROUTINE ewald_abcm(atoms,mass,charge,rmesh,kmesh,alpha,vol,qvec,dyn_abcm,N,NION,nr,nk)
    ...
    real(8), intent(in) :: atoms(N,3),rmesh(nr,3),kmesh(nk,3)
    ! mass.shape=(N,) so is charge, qvec.shape=(3,)
    real(8), intent(in) :: mass(NION),charge(N),qvec(3)
    real(8), intent(in) :: alpha,vol
    integer, intent(in) :: N,NION,nr,nk
    ! ============output============ !
    complex(8),dimension(3*NION,3*NION),intent(out) :: dyn_abcm
    ! ============local============= !
    ! force constant tensor
    real(8), dimension(3,3) :: fc,fc1
    ! temporary variables
...
!             IF (n1 .EQ. n2) CYCLE
    ...
    ! ABCM operation
    dyn_abcm(:,:) = dyn(1:3*NION,1:3*NION) - MATMUL( dyn(1:3*NION,3*NION+1:3*N),MATMUL( S,dyn(3*NION+1:3*N,1:3*NION) ) )
    !
    dyn_abcm(:,:) = MATMUL(mmat,dyn_abcm(:,:))
    !
